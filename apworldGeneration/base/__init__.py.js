export const initpy = (game) => {
	return `
from BaseClasses import Region, Item, ItemClassification, logging
...
from .options import rgOptions, rg_option_groups
from .rgCollections import rgCollections
from .items import rgSongItem, rgFixedItem
from .locations import rgLocation

class rgWeb(WebWorld):
    theme = "stone"
    option_groups = rg_option_groups
...
class rgWorld(World):
    ...
    """
    game = "${game}"
    options_dataclass: ClassVar[Type[PerGameCommonOptions]] = rgOptions
    options: rgOptions
    ...
    rg_collection = rgCollections()
    filler_item_names = list(rg_collection.filler_items.keys())
    filler_item_weights = list(rg_collection.filler_weights.values())

    item_name_to_id = {name: code for name, code in rg_collection.item_names_to_id.items()}
    location_name_to_id = {name: code for name, code in rg_collection.location_names_to_id.items()}
    item_name_groups = rg_collection.getItemNameGroups()
    ...
    def generate_early(self):
        ...
        if re_gen_passthrough and self.game in re_gen_passthrough:
            ...
            if "finalSongIDs" in slot_data:
                ...
                self.included_songs = [key for key, song in self.rg_collection.song_items.items() if song.song_id in final]
        ...
        while True:
            available_song_keys = self.rg_collection.getSongsWithSettings(self.options, min_diff, max_diff)
    ...
    def create_item(self, name: str) -> Item:
        if name == self.rg_collection.SHEET_NAME:
            return rgFixedItem(name, ItemClassification.progression_skip_balancing,
                                 self.rg_collection.SHEET_CODE, self.player)
        
        filler = self.rg_collection.filler_items.get(name)
        if filler:
            return rgFixedItem(name, ItemClassification.filler, filler, self.player)
        
        song = self.rg_collection.song_items[name]
        self.final_song_ids.add(song.title)
        return rgSongItem(name, self.player, song)
    ...
    def create_items(self) -> None:
        ...
        for _ in range(0, item_count):
            self.multiworld.itempool.append(self.create_item(self.rg_collection.SHEET_NAME))
    ...
    def create_regions(self) -> None:
        ...
        for name in all_selected_locations:
            for j in range(2):
                loc = rgLocation(self.player, f"{name}-{j}", self.rg_collection.song_locations[f"{name}-{j}"], menu_region)
                loc.access_rule = lambda state, item=name: state.has(item, self.player)
                menu_region.locations.append(loc)

    def set_rules(self) -> None:
        self.multiworld.completion_condition[self.player] = lambda state: \\
            state.has(self.rg_collection.SHEET_NAME, self.player, self.get_sheet_win_count())
    ...
    def fill_slot_data(self):
        return {
            ...
            "sheetName": self.rg_collection.SHEET_NAME,
